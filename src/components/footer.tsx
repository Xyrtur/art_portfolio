...
function FooterLink({hrefName, name}:{hrefName:string, name:string}) { return(<Link href={hrefName} className="hover:text-link-color hover:underline px-6 pb-3" target={name == "Instagram" ? "_blank" : undefined}>
    {name}
    </Link>);
    }
...
const Footer = () => {
    return (
        <footer className="absolute bottom-0 w-full text-[0.9rem]">
            <div className="flex flex-col justify-center">
                <div className="grid grid-cols-6 gap-x-8 grid-flow-col w-2/3 place-self-center" >
                    ...
                    <FooterLink hrefName="https://www.instagram.com/scraped_art.yzzy/" name="Instagram"/>

        
                </div>
                <hr className="bg-white h-0.5 border-none mt-8 place-self-center w-5/6 "/>
                <p className="text-center pb-8 pt-3">Copyright Ysabelle Kmiec ©2024. All Rights Reserved.</p>
            </div>
        </footer>
    );
}
